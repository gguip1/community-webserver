...
- Run the runner as a dedicated non-login Linux user with only the Docker permissions required for deployment.
...
Create `/etc/wepick/prod.env` with root ownership and mode `600`. It contains only runtime secrets and stable values, not image tags:
